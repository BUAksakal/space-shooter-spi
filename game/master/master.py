...
def startup():
    ...
    # Wait: both joystick buttons pressed simultaneously = start hosting
    print("[Pi A] Waiting for both joystick buttons to host...")
    while True:
        b7  = not GPIO.input(GPIO_SHOOT)    # Joystick B7 (GPIO5)
        bb  = not GPIO.input(GPIO_JOY_BB)   # Joystick BB (GPIO6)
        if b7 and bb:
    ...
    # Send JOIN_REQUEST, wait for JOIN_ACK from Pi B
    while True:
